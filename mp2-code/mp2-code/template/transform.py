...
def transformToMaze(arm, goals, obstacles, window, granularity):
    ...
    if len(arm.getArmLimit()) == 1:
        ...
        for i in range(rowNum):
            # print("1")
            angles = [i * granularity + min(armLimit[0][0], armLimit[0][1])]
            # print(angles)
            arm.setArmAngle(angles)
            if doesArmTouchObstacles(arm.getArmPos(), obstaclesAll) or not isArmWithinWindow(arm.getArmPos(),
                                                                                             window):
            ...
            if doesArmTouchGoals(arm.getEnd(), goals) and not (
                    doesArmTouchObstacles(arm.getArmPos(), obstacles) or not isArmWithinWindow(arm.getArmPos(),
                                                                                               window)):
                # if transformMap[i][ii] == "P":
                #     print("??")
                #     print(arm.getArmPos(), window)
                if transformMap[i] == "%":
                    print("ops, this point is counted to both obstacles/outside and goals", i)
                transformMap[i] = "."
        transformMap[round((initArmAngles[0] - min(armLimit[0][0], armLimit[0][1])) / granularity)] = "P"

        # transformMap[1][1] = "P"
        # transformMap[1][2] = "."
        offset = [min(armLimit[0])]
        retMaze = Maze(transformMap, offset, granularity)

        outputMap = ""
        for alpha in range(len(transformMap)):
            outputMap += transformMap[alpha]
        # outputMap += "\n"
        print(outputMap)
    ...
    if len(arm.getArmLimit()) == 3:
        ...
        rowNum = round(abs(armLimit[0][0] - armLimit[0][1]) / granularity + 1)
        colNum = round(abs(armLimit[1][0] - armLimit[1][1]) / granularity + 1)
        verNum = round(abs(armLimit[2][0] - armLimit[2][1]) / granularity + 1)
        transformMap = []
        # print(rowNum, colNum)
        # print(granularity)
        for i in range(rowNum):
            ...
            for ii in range(colNum):
                transformMap[i].append([])
                for iii in range(verNum):
                    transformMap[i][ii].append(" ")
        ...
        for i in range(rowNum):
            for ii in range(colNum):
                for iii in range(verNum):
                    # print("1")
                    angles = [i*granularity + min(armLimit[0][0], armLimit[0][1]), ii*granularity + min(armLimit[1][0], armLimit[1][1]), iii*granularity + min(armLimit[2][0], armLimit[2][1])]
                    # print(angles)
                    arm.setArmAngle(angles)
                    if doesArmTouchObstacles(arm.getArmPos(), obstaclesAll) or not isArmWithinWindow(arm.getArmPos(), window):
                        # if transformMap[i][ii] == "P":
                    ...
                    if doesArmTouchGoals(arm.getEnd(), goals) and not (doesArmTouchObstacles(arm.getArmPos(), obstacles) or not isArmWithinWindow(arm.getArmPos(), window)):
                        ...
                        #     print("??")
                        #     print(arm.getArmPos(), window)
                        if transformMap[i][ii][iii] == "%":
                            print("ops, this point is counted to both obstacles/outside and goals", i, ii)
                        transformMap[i][ii][iii] = "."
        transformMap[round((initArmAngles[0] - min(armLimit[0][0], armLimit[0][1])) / granularity)] \
                    [round((initArmAngles[1] - min(armLimit[1][0], armLimit[1][1])) / granularity)] \
                    [round((initArmAngles[2] - min(armLimit[2][0], armLimit[2][1])) / granularity)] = "P"


        # transformMap[1][1] = "P"
        # transformMap[1][2] = "."
        offset = [min(armLimit[0]), min(armLimit[1]), min(armLimit[2])]
        retMaze = Maze(transformMap, offset, granularity)


        outputMap = ""
        for gamma in range(len(transformMap[0][0])):
            for beta in range(len(transformMap[0])):
                for alpha in range(len(transformMap)):
                    outputMap += transformMap[alpha][beta][gamma]
                outputMap += "\n"
            outputMap += "\n\n"

        print(outputMap)
        print(len(transformMap), len(transformMap[0]), len(transformMap[0][0]))
